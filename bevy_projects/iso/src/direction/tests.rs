...
mod edge_directions {
    ...
    fn flat_angles_rad() {
        ...
        for (dir, angle) in expected {
            ...
            assert_relative_eq!(dir.angle(ProjectionMatrix::Top), angle);
        }
    }
    ...
    fn pointy_angles_degrees() {
        ...
        for (dir, angle) in expected {
            ...
            assert_relative_eq!(dir.angle_degrees(ProjectionMatrix::Full), angle);
        }
    }
    ...
    fn pointy_angles_rad() {
        ...
        for (dir, angle) in expected {
            ...
            assert_relative_eq!(dir.angle(ProjectionMatrix::Full), angle);
        }
    }
...
mod vertex_direction {
    ...
    fn flat_angles_degrees() {
        ...
        for (dir, angle) in expected {
            assert_relative_eq!(dir.angle_flat_degrees(), angle);
            assert_relative_eq!(dir.angle_degrees(ProjectionMatrix::Top), angle);
        }
    }
...
#[test]
fn rad_deg() {
    for orientation in [ProjectionMatrix::Top, ProjectionMatrix::Full] {
        for i in 0..6 {
            let edge = EdgeDirection(i);
            let vertex = VertexDirection(i);
