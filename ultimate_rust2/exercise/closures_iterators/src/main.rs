...
fn main() {
    ...
    println!("{:?}", numbers.into_iter().map(|x| x * 3).collect::<Vec<i32>>()); // should print [3, 6, 9, 12]
    ...
    let transformed: Vec<_> = words.into_iter().filter(|x| !x.contains("h")).map(|x| x.to_uppercase()).collect();  // do the stuff here
